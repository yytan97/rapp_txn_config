...
import { cleanUp as cleanUp4Detail } from "./SessionDetailPage.js";
...
export function SessionManagementPage({ debugMode = true }) {
    const componentName = "SessionManagementPage";
    ...
    */
        ...
        try {
            ...
            if (reset) {
                ...
                */
                ...
                let result2 = await apiBox.createSessionCursor(getSessionToken(), searchObject.searchText);
                if (result2.flag && result2.data) {
                    if (result2.data?.cursor == undefined) throw { errorCode: 9006 };
            ...
            fixPage();
            let result4 = await apiBox.rewindNFetch4Session(getSessionToken(), cursorId, pageObject.page, pageObject.pageSize);
    ...
    function click4DeleteRecord(e, record, index) {
        ...
        let message = sl.m_confirm_delete_session;
        ...
        showConfirmDialogBox(message, async () => {
            ...
            showStateDialogBox();
            try {
                let result1 = await apiBox.deleteRecord4Session(getSessionToken(), record.name);

                if (result1 && result1.flag) {

                    let message = sl.m_session_deleted;
                    showInfoDialogBox(message, () => {
                        return;
                    });
                                    ...
                                    <thead>
                                        <tr className="text-nowrap" style={{ fontSize: "12px", color: "#A4A6A7", fontWeight: "600" }} >
                                            ...
                                            <th className="text-end">
                                                {sl.h_active_sessions}
                                            </th>

                                            <th className="">
                                                {sl.h_last_active_timestamp}
                                            </th>
