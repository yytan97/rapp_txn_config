...
export function EditTeamPage({ debugMode = true }) {
    const componentName = "EditTeamPage";
    ...
    function click4AddRecord(e) {
        ...
        let message = sl.m_confirm_add_team;
        message = message.replace("__parameter_1", inputData.teamName);
        showConfirmDialogBox(message, async function () {
            showStateDialogBox();
            try {

                let result1 = await apiBox.addRecord4UserGroup(getSessionToken(), inputData.teamName);
                if (result1 && result1.flag) {
                    formObject.dirty = false;

                    let message = sl.m_team_added;
                ...
                <div className="col-8" style={{ minHeight: "50vh" }}>
                    ...
                    <div className="pb-3 border-bottom">
                        <div style={{ color: "#242627", "fontSize": "16px", fontWeight: "bold" }} >
                            {(editMode === 0) ? sl.l_create_new_team : sl.l_edit_cryptogram}
                        </div>
                    ...
                    <div className="px-4 mt-4">
                        ...
                        <div className="my-3 px-3">
                            ...
                            <div>
                                <InputLabel label={sl.l_team_name} required />
                                <input name="teamName"
                                    type="text"
                                    className={`form-control ${tBox.getClass4IsInvalid2('teamName', formObject)}`}
                                    placeholder={sl.p_your_team_name}
                                ...
                                <ErrorLine message={tBox.getFieldErrorMessage2('teamName', sl, formObject)} />
                            </div>
